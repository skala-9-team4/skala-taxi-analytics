...
def run_advanced_pipeline(df: pd.DataFrame, target_col: str):
    ...
    results_df["is_best"] = results_df["model"].eq(best_version)

    ml_output_dir = Path("reports/generated/ml")
    ml_output_dir.mkdir(parents=True, exist_ok=True)

    results_path = ml_output_dir / "model_results.csv"
    ...
    print(f"모델 성능 결과 저장 완료: {results_path}")
